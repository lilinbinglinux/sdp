...
* 类描述：   字符串操作方法
* 创建人：姚林刚   
* 创建时间：2015年3月11日 下午5:00:43   
* 修改人：Administrator   
* 修改时间：2015年3月11日 下午5:00:43   
* 修改备注：   
* @version
 */
public class StringUtil {
    ...
    // 字符串转为被xml借些  condition
    public static String stringToXmlString(Object str) {
	...
	public static String getSmsContent(String messages, String insertString,
			int size1) {
			...
			for (int s = 0; s < size; s++) {
				...
				if (s == size - 1) {
					...
					if (lastSms.length() <= size1) {
						mes.add(lastSms);
					} else if (lastSms.length() > size1
							&& lastSms.length() <= size1 * 2) {
	...
	/*public static void main(String arg[]){
		...
		System.out.println(getDateString(-9, "-"));

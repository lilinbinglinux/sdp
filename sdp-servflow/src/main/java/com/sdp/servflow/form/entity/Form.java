package com.sdp.servflow.form.entity;

import java.math.BigDecimal;
import java.util.Date;

public class Form {
	
	private String id;
	
	private String tablename;
	
	private String tablecnname;
	
	private String state;
	
	private String remark;
	
	private String text;
	
	private String organcode;
	
	private String delflag;
	
	private Date subtime;
	
	private String templatescript;
	
	private String version;
	
	private String createid;
	
	private String udtemplatecode;
	
	private String appflag; 
	
	private String fieldtitle;
	
	private String exampletitle;

	public String getId() {
		return id;
	}

	public void setId(String id) {
		this.id = id;
	}

	public String getTablename() {
		return tablename;
	}

	public void setTablename(String tablename) {
		this.tablename = tablename;
	}

	public String getTablecnname() {
		return tablecnname;
	}

	public void setTablecnname(String tablecnname) {
		this.tablecnname = tablecnname;
	}

	public String getState() {
		return state;
	}

	public void setState(String state) {
		this.state = state;
	}

	public String getRemark() {
		return remark;
	}

	public void setRemark(String remark) {
		this.remark = remark;
	}

	public String getText() {
		return text;
	}

	public void setText(String text) {
		this.text = text;
	}

	public String getOrgancode() {
		return organcode;
	}

	public void setOrgancode(String organcode) {
		this.organcode = organcode;
	}

	public String getDelflag() {
		return delflag;
	}

	public void setDelflag(String delflag) {
		this.delflag = delflag;
	}

	public Date getSubtime() {
		return subtime;
	}

	public void setSubtime(Date subtime) {
		this.subtime = subtime;
	}

	public String getTemplatescript() {
		return templatescript;
	}

	public void setTemplatescript(String templatescript) {
		this.templatescript = templatescript;
	}

	public String getVersion() {
		return version;
	}

	public void setVersion(String version) {
		this.version = version;
	}

	public String getCreateid() {
		return createid;
	}

	public void setCreateid(String createid) {
		this.createid = createid;
	}

	public String getUdtemplatecode() {
		return udtemplatecode;
	}

	public void setUdtemplatecode(String udtemplatecode) {
		this.udtemplatecode = udtemplatecode;
	}

	public String getAppflag() {
		return appflag;
	}

	public void setAppflag(String appflag) {
		this.appflag = appflag;
	}

	public String getFieldtitle() {
		return fieldtitle;
	}

	public void setFieldtitle(String fieldtitle) {
		this.fieldtitle = fieldtitle;
	}

	public String getExampletitle() {
		return exampletitle;
	}

	public void setExampletitle(String exampletitle) {
		this.exampletitle = exampletitle;
	}
	
}

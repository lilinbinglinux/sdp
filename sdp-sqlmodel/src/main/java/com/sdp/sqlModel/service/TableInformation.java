...
public class TableInformation implements Runnable{

	private final static Logger 		logger= LoggerFactory.getLogger(TableInformation.class);
	...
	public void createTable(){
		for (Map<String,Object> table : tableNames) {
			...
			sqlTable.setTableSqlName(table.get("TABLE_NAME").toString());
			sqlTable.setTableName(table.get("TABLE_NAME").toString());
			sqlTable.setSortId(0);
			if(table.get("TABLE_COMMENT") != null && !table.get("TABLE_COMMENT").toString().equals("")){
				sqlTable.setTableResume(table.get("TABLE_COMMENT").toString());
			}
			sqlTable.setCreateBy(userInfo.getLoginId());
			sqlTable.setDataResId(sqlDataRes.getDataResId());
			sqlTable.setTenantId(userInfo.getTenantId());
			sqlTable.setTableTypeId(sqlTableType.getTableTypeId());
			sqlTableMapper.savesqltable(sqlTable);
			...
			if(sqlDataResType.getResType().equals("1")){//mysql
				...
				if(tableInfoes != null && !tableInfoes.isEmpty()){
    				fieldMysql( tableInfoes, table, sqlTable, userInfo,sqlDataRes);
    			}
	    		...
	    		if(tableInfoes != null && !tableInfoes.isEmpty()){
	    			fieldCreate( tableInfoes, table, sqlTable, userInfo, sqlDataResType,sqlDataRes);
    			}
...
//	    			tableInfoes = sqlDataResDao.getTableInfoesSqlserver(table.get("TABLE_NAME").toString());
	 ...
	 */
	 public void createForegin(List<Map<String,Object>> tableNames,User userInfo,SqlDataResType sqlDataResType,SqlTableType sqlTableType,SqlDataRes sqlDataRes){
	     ...
	     * @param sqlDataResType
	     */
	    public void fieldCreate(List<Map<String,Object>> tableInfoes,Map<String,Object> table,SqlTable sqlTable,User userInfo,SqlDataResType sqlDataResType,SqlDataRes sqlDataRes){
	    	//查询主外键信息--更新field与添加外键表
	    	List<String> keyInfo = null;
	    	List<String> keyInfoes = null;
	    	try{
				DynamicDataSourceContextHolder.setDataSourceType(sqlDataRes.getDataResId());
				if(sqlDataResType.getResType().equals("2")){//oracle
		    		keyInfo = sqlDataResDao.getPrimaryKeyOracle(table.get("TABLE_NAME").toString());
//		    		keyInfoes = sqlDataResDao.allKeyInfoesOracle(table.get("TABLE_NAME").toString());
		    	}else if(sqlDataResType.getResType().equals("3")){//DB2
		    		keyInfo = sqlDataResDao.getPrimaryKeyDb(table.get("TABLE_NAME").toString());
//		    		keyInfoes = sqlDataResDao.allKeyInfoesDb(table.get("TABLE_NAME").toString());
		    	}else if(sqlDataResType.getResType().equals("5")){//sqlserver
		    		keyInfo = sqlDataResDao.getPrimaryKeySqlserver(table.get("TABLE_NAME").toString());
//		    		keyInfoes = sqlDataResDao.allSordexInfoesSqlerver(table.get("TABLE_NAME").toString());
		    	}
			}catch(Exception e){
				e.printStackTrace();
			}finally{
	     ...
	     * @param userInfo
	     */
	    public void fieldMysql(List<Map<String,Object>> tableInfoes,Map<String,Object> table,SqlTable sqlTable,User userInfo,SqlDataRes sqlDataRes){
			//查询主外键信息--更新field与添加外键表
	    	List<String> keyInfo = null;
	    	try{
				DynamicDataSourceContextHolder.setDataSourceType(sqlDataRes.getDataResId());
				keyInfo = sqlDataResDao.getPrimaryKeyMysql(table.get("TABLE_NAME").toString(), sqlDataRes.getTableSchema());
//				List<Map<String,Object>> keyInfoes = sqlDataResDao.allKeyInfoesMysql(table.get("TABLE_NAME").toString(),sqlDataRes.getTableSchema());
			}catch(Exception e){
				e.printStackTrace();
			}finally{
				DynamicDataSourceContextHolder.clearDataSourceType();
			...
			for (Map<String, Object> map : tableInfoes) {
				...
				if(keyInfo != null && !keyInfo.isEmpty() && keyInfo.contains(map.get("COLUMN_NAME").toString())){
					...
					//添加主键约束字段
//					for (Map<String, Object> map2 : keyInfoes) {
//						if(map2.get("COLUMN_NAME").toString().equals(map.get("COLUMN_NAME").toString())){
							//sqlField.setnew(map2.get("CONSTRAINT_NAME"));//约束名称
//						}
//					}
				}else{
					sqlField.setFieldKey("0");

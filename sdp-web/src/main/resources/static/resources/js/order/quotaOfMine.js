		...
		$.ajax({
			...
			success: function (result) {
				if( result ){
					...
					if( list.length > 0 ){
						var curData = list[0];
						var tenantId = curData.tenantId;

						var resQuota = curData.resQuota;
						var usedQuota = curData.usedQuota;

						for( var key in resQuota ){
							var curRes = resQuota[key];
							var curUsed = usedQuota[key];
							var html =
								'<div class="quota-detail-list">';
							html += '<div class="quota-item quota-item-title">'+key+'</div>';
							html += '<div class="quota-item quota-item-process">\n' +
								'       <div class="progress">\n' +
								'         <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="'+curUsed+'" aria-valuemin="0" aria-valuemax="'+curRes+'" style="width: '+(curRes === 0 ? 0 : ( (curUsed/curRes)*100 ).toFixed(2))+'%;">'+(curRes === 0 ? 0 : ( (curUsed/curRes)*100 ).toFixed(2))+'%</div>\n' +
								'       </div>\n' +
								'     </div>';
							if( key === "CPU" ){
								html += '<div class="quota-item quota-item-content">'+ curUsed +'/'+ curRes +'(个)</div>';
							}else if( key === "内存" ){
								html += '<div class="quota-item quota-item-content">'+ curUsed +'/'+ curRes +'(G)</div>';
							}else if( key === "硬盘" ){
								html += '<div class="quota-item quota-item-content">'+ curUsed +'/'+ curRes +'(G)</div>';
							}else{
								html += '<div class="quota-item quota-item-content">'+ curUsed +'/'+ curRes +'</div>';
							}
							html += '</div>';
							$("#res-used").append(html);
						}

						loadquotaDetailTable(tenantId);
					}
				}
			}
		});

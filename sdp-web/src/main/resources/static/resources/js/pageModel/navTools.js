...
function cleanLayout( e ) {
    if ( e ) {
        ...
        try {
            let nodeRes = tag.getAttribute( 'data-moduleres' );
            if ( nodeRes ) {
                nodeRes.split( ',' ).map( res => res.split( ':' ) ).forEach( function( node ) {
                    let count = $( '#' + node[1] ).attr( 'data-count' );
...
function initEditorial( e ) {
    let preparedTarget = findParentDragHelper( e );
    let id = preparedTarget.getAttribute( 'data-uuid' );
    let eJs = $( '#' + id + 'js' ).html() || '';
    let eCs = $( '#' + id + 'cs' ).html() || '';
    let eText = $( '#v-pureHtmlTemplate' ).contents().find( '[data-uuid="' + id + '"]' ).prop( 'outerHTML' );
...
function saveEditorial() {
    let editorNodeIds = window.preparedTarget.getAttribute( 'data-uuid' );
    let mainCs = $( '#' + editorNodeIds + 'cs' );
    let mainJs = $( '#' + editorNodeIds + 'js' );

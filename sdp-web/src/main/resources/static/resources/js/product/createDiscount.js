            ...
            if(data.productAttrOrm == null) {
                ...
                $.each(data.productAttrOrm,function (index,item) {
                    ...
                    if(item.proLabel.indexOf(30) !== -1) { // 资源
                        rescount++;
                        controlResAttrHtml += setResDiscount(item,resIndex);
                        resIndex++;
                ...
                $('#packageControlResAttr input[type="checkbox"]').on('change',function () {
                    if($(this).is(':checked')) {
                        $(this).parent().parent().siblings('.newPrice').css('display','inline-block');
                    }else{
                        $(this).parent().parent().siblings('.newPrice').hide();
                    }
                })
            }
...
// 保存及发布折扣
function handleSaveDiscount(packageStatus) {
...
function getAllPorderWays(orderType) {
    $.ajax({
        ...
        success: function (result) {
            ...
            $("#packageAddForm").bootstrapValidator("addField", "orderType", { //对动态生成的表单添加校验
                validators: {
                    notEmpty: {
                        message: '请选择订购方式'
                    }

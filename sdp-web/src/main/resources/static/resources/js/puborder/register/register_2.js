...
$(document).ready(function () {
    ...
    if(flowChartId == null || flowChartId == ""){
        idflowChartId = tag.getData(flowChartId).flowChartId;
        updateflag = "";
        //先添加服务
        initStartLayout();
        initBaseinfoNull();
...
function initBaseinfo() {
    for (var i = 0; i < serNodeArray.length; i++) {
        if(serNodeArray[i].nodeName == "开始"){
            baseserNodeArray = serNodeArray[i];
        }
    }
...
function changeTag(type, tablefalg) {
    ...
    if ($("#pubname").val() == "" || $("#pubname").val() == null) {
        ...
        baseserNodeObj.inCharset = $("#inCharset").val();
        baseserNodeObj.outCharset = $("#outCharset").val();

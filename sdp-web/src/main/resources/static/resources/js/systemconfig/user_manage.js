...
function initUserTable(){
	$("#userTable").width("100%").dataTable({
		...
		},
		{
			...
			"render" : function(row) {
				if(row != null){
					var html = formatDate(row,"yyyy-MM-dd HH:mm:ss");
					return html;
				}
			}
		},
		...
		success:function(data){
			...
			if(data != null&&data != undefined){
				for(var i=0;i<data.length;i++){
					var optionText = "<option value='"+ data[i].tenantName +"'>"+data[i].tenantName+"</option>";
					$("#tenantIdVal").append(optionText); 
				}
			}
		}
	});

...
(function ($)
{
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            if(g.image){
            	g.wrapper.append('<img class="aui-ss-entity-icon" src="../image/people.png">');
            }
            ...
            g.selectBox.hover(null, function (e)
            {
                ...
                {
//                	duanhan  修改当鼠标移开selectBox时消失，这种用户体验度不高。注释掉这句话，使selectBox 发生点击事件后消失。
...
//                	20161208duanhan   修改点击父节点时没有选中问题.
//                    if (p.treeLeafOnly && node.data.children) return;
...
(function ($)
{
    ...
    //dialog 图片文件夹的路径 预加载
    $.ligerui.DialogImagePath = "../../lib/ligerUIskins/Aqua/images/win/";
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        switchPageSizeApplyForm: true,     //切换每页记录数是否应用ligerForm
        ...
        async: true,
        fixedCellHeight: false,                       //是否固定单元格的高度   duanhan修改 取消固定单元格高度
        ...
        enabledSort: false,                      //是否允许排序 duanhan改成了默认不排序
...
//      pageStatMessage: '显示从{from}到{to}，总 {total} 条 。每页显示：{pagesize}',
        pageStatMessage: '{pagesize}条/页，共 {total} 条 记录',
...
//        isContinueByDataChanged: '数据已经改变,如果继续将丢失数据,是否继续?',
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("            <span class='l-bar-message'><span class='l-bar-text'></span></span>");
            gridhtmlarr.push("            <span class=' l-bar-selectpagesize'></span>");
          /*  gridhtmlarr.push("            <span class=' l-bar-selectpageno'></span>");*/
            gridhtmlarr.push("            <div class='l-bar-group'>");
            gridhtmlarr.push("                <button class='l-bar-button  l-bar-btnfirst'>首页</button>");
            gridhtmlarr.push("                <button class='l-bar-button l-bar-btnprev'>上一页</button>");
            gridhtmlarr.push("                <span class='pcontrol'> </span>");
            gridhtmlarr.push("                <button class='l-bar-button  l-bar-btnnext'>下一页</button>");
            gridhtmlarr.push("                <button class='l-bar-button  l-bar-btnlast'>尾页</button>");
            gridhtmlarr.push("                <button class='l-bar-button  l-bar-btnload'>刷新</button>");
            gridhtmlarr.push("            </div>");
...
//                g.loadServerData.ligerDefer(g, 10, [param, clause]);
                ...
                success: function (data)
                {
                    ...
                    if (!data || !data[p.root] || !data[p.root].length)
                    {
                        ...
                        if (data && data[p.record])
                        {
                        	...
                        	g.currentData[p.root] = g.data[p.root] = data[p.root];
                        } else
                        {
        ...
        */
        ...
        //改变分页
        changePage: function (ctype,e)
        {
        	var g = this, p = this.options;
        	var btnPage = "";
        	if(ctype=='button'){
    			btnPage = e.target.innerHTML;
        	}
        	$('.pcontrol' ,g.toolbar).empty();
            if (g.loading) return true;
//            if (p.dataAction != "local" && g.isDataChanged && !confirm(p.isContinueByDataChanged))
            //改变行数据后，认为应该不弹出“数据已经改变,如果继续将丢失数据,是否继续”
            /*if (p.dataAction != "local" && g.isDataChanged && confirm(p.isContinueByDataChanged))
                return false;*/
            switch (ctype){
            	case 'first': 
        	 		for(var i = 1;i<=p.pageCount;i++){
        	 			if(i>5)break;
        	 			$('.pcontrol',g.toolbar).append('<button class="l-bar-button" id="page-button-'+i+'">'+i+'</button>');	
        	 		}
        	 		p.newPage = 1;
            	 	 break;
	            case 'prev': 
		        	...
		        	if (p.page > 1){
	        		 ...
	        		 var pnum = p.newPage+4;
			        	if(pnum>p.pageCount){
			            	for(var i = p.pageCount-4;i<=p.pageCount;i++){
				            	if(i>(pnum))break;
				            	if(i<=0)break;
				      	 		$('.pcontrol',g.toolbar).append('<button class="l-bar-button" id="page-button-'+i+'">'+i+'</button>');	
				      	 	}
			            }else{
			            	for(var i = p.newPage;i<=p.pageCount;i++){
				            	if(i>(pnum))break;
				      	 		$('.pcontrol',g.toolbar).append('<button class="l-bar-button" id="page-button-'+i+'">'+i+'</button>');	
				      	 	}
			            }
	            ...
	            case 'next': 
		            ...
		            break;
	            case 'last': 
		            if (p.page > p.pageCount) return; p.newPage = p.pageCount; 
		            $('.pcontrol',g.toolbar).append('<button class="l-bar-button" id="page-button-'+p.pageCount+'">'+p.pageCount+'</button>');
		            break;
	            case 'button':
	            	var nv =  parseInt(btnPage);
	            	p.newPage = nv;
	            	var pnum = p.newPage+4;
		            if(pnum>p.pageCount){
		            	for(var i = p.pageCount-4;i<=p.pageCount;i++){
            ...
            if (p.newPage == 1)
            {
                $(".l-bar-btnfirst", g.toolbar).addClass("l-disabled").attr("disabled","disabled");
                $(".l-bar-btnprev", g.toolbar).addClass("l-disabled").attr("disabled","disabled");
            }
            else
            {
                $(".l-bar-btnfirst", g.toolbar).removeClass("l-disabled").attr("disabled",false);
                $(".l-bar-btnprev", g.toolbar).removeClass("l-disabled").attr("disabled",false);
            }
            if (p.newPage == p.pageCount)
            {
            	$(".l-bar-btnlast", g.toolbar).addClass("l-disabled").attr("disabled","disabled");
                $(".l-bar-btnnext", g.toolbar).addClass("l-disabled").attr("disabled","disabled");
            }
            else
            {
                $(".l-bar-btnlast", g.toolbar).removeClass("l-disabled").attr("disabled",false);
                $(".l-bar-btnnext ", g.toolbar).removeClass("l-disabled").attr("disabled",false);
            }
        ...
        {
            var g = this, p = this.options;
            var jcell = $("<td class='l-grid-hd-cell'><div class='l-grid-hd-cell-inner' style='height: 32px;'><span class='l-grid-hd-cell-text'></span></div></td>");
        ...
        _initFootbar: function ()
        {
            ...
            if (p.usePager)
            {
              ...
              //创建底部工具条 - 选择每页显示记录数
               ...
               //将选择每页条数的选择数组换成input duanhan 
                $('.l-bar-selectpagesize', g.toolbar).append("<span>每页</span><input type='text' name='rp' value="+p.pageSize+"><span>条</span>");
                $('.l-bar-selectpageno', g.toolbar).append("<span>跳转到</span><input type='text' id='spn' /><span>页</span>");
//                if (selectedIndex != -1) $('.l-bar-selectpagesize select', g.toolbar)[0].selectedIndex = selectedIndex;
            /*    if (p.switchPageSizeApplyComboBox && $.fn.ligerComboBox)
                ...
                $(".l-bar-selectpagesize input[name=rp]", g.toolbar).width("45px");
                $(".l-bar-selectpageno input", g.toolbar).width("45px");
        ...
        _showData: function (sourceType)
        {
            ...
            if (p.usePager)
            {
                ...
                if (!p.scrollToPage)
                {
                	$('.pcontrol',g.toolbar).empty();
                	if(p.pageCount>5){
                		if(p.pageCount-p.page<4){
                			for(var i = p.pageCount-4;i<=p.pageCount;i++){
                                $('.pcontrol',g.toolbar).append('<button class="l-bar-button" id="page-button-'+i+'">'+i+'</button>');
                        	}
                		}else{
                			for(var i = p.page;i<=p.pageCount;i++){
                                if(i>p.page+4){
                                	break;
                                }
                                $('.pcontrol',g.toolbar).append('<button class="l-bar-button" id="page-button-'+i+'">'+i+'</button>');
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                else if (p.tree && p.tree.isExtend)
                {
                    var isHide = function ()
                    {
                        ...
                        while (pitem)
                        {
                            if (p.tree.isExtend(pitem) == false){	
								return true;
							}
                            pitem = g.getParent(pitem);
                        }
                ...
                $(g.columns).each(function (columnindex, column)
                {
                    ...
                    if (this.isrownumber)
                    {
                        ...
                        if (p.fixedCellHeight)
                            gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px; margin-top:5px;"');
                        else
        ...
        _getTreeCellHtml: function (oldContent, rowdata)
        {
           ...
           // var isExtend = p.tree.isExtend(rowdata);
			var isExtend =   p.tree.isExtend != null ? p.tree.isExtend(rowdata) : $.inArray(rowdata, g.collapsedRows || []) == -1;
        ...
        _buildPager: function ()
        {
            ...
            for(var i = 0;i<$(".pcontrol button").length;i++){
            	var $this = $(".pcontrol button").eq(i);
            	if(parseInt($this.text())==p.page)
            		{
            			$this.css("background-color","#0070d2");
            			$this.css("color","#fff");
            		}
            }
            if (!p.pageCount) p.pageCount = 1;
        ...
        _setEvent: function ()
        {
            ...
            //工具条 - 切换每页记录数事件
//            $('select', g.toolbar).change(function ()   duanhan
            $(".l-bar-selectpagesize input[name=rp]", g.toolbar).change(function ()
            {
//            	if (g.isDataChanged && p.dataAction != "local" && !confirm(p.isContinueByDataChanged))
            ...
            $(".l-bar-selectpageno input", g.toolbar).change(function ()
            {
//            	if (g.isDataChanged && p.dataAction != "local" && !confirm(p.isContinueByDataChanged))
            	if (g.isDataChanged && p.dataAction != "local" && confirm(p.isContinueByDataChanged))
                    return false;
                p.newPage = this.value;
                
                g.loadData(p.dataAction != "local" ? p.where : false);
            });
            //工具条 - 切换当前页事件
            $('.pcontrol ', g.toolbar).click(function (e)
            {
            	g.changePage('button',e);
            });
...
//                    if (p.url && g.isDataChanged && !confirm(p.isContinueByDataChanged))
        ...
        _onResize: function ()
        {
            ...
            if( g.chkReSize==0){
            	var cwidth = p.width;
            	if(cwidth=='auto'||(typeof (cwidth) == "string" &&cwidth.indexOf("%")>0)){
            		var cw = $(document).width();
            		if(cwidth.indexOf("%")!=-1){
            			cw = cw*(parseInt(cwidth)*0.01);
            		}else{
            			cw *= 0.98;
            		}
            		g.chkReSize=1;
                    g.autoResizeWidth(cw);
                    g.chkReSize=0;
            	}else{
            		g.chkReSize=1;
                    g.autoResizeWidth(cwidth);
                    g.chkReSize=0;
            	}
            }
        ...
        autoResizeWidth:function(ChangedWidth) {
        	return false;
            var g = this;
            $(g.element).css({width:ChangedWidth});
            for (var level = 1; level <= g._columnMaxLevel; level++)  
            {     
                //1、修改column宽度  
                var columns = g.getColumns(level);           //获取level层次的列集合  
                ...
                $(columns).each(function (i, column)  
                {     
                	 if(column.hide || column.ischeckbox){//扣除序号列和隐藏列  
                         colNum = colNum - 1;  
                         ChangedWidth  = ChangedWidth-30;
                     } 
                    //排除每列自定义
                    var cwi = column.width;
                	if(cwi=='auto'||(typeof (cwi) == "string" &&cwi.indexOf("%")>0)||cwi!=undefined){
                         if(column.isrownumber ){//扣除序号列
                             colNum = colNum - 1;  
                             ChangedWidth  = ChangedWidth-30;
                         } else{
                    		 colNum = colNum - 1;  
                             ChangedWidth  = ChangedWidth-cwi;
                         }
...
(function ($)
{
    ...
    $.ligerui.controls.ListBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _addDblClickEven: function ()
        {
            var g = this, p = this.options;
            //选项点击
            g.selectBox.dblclick(function (e)
            {  
                var obj = (e.target || e.srcElement); 
                var jrow = $(obj).parents("tr:first");
                if (!jrow.length) return;
                ...
                jrow.find(":checkbox").each(function () { this.checked = jrow.hasClass("l-selected"); });
                g._updateValue();
                g.trigger('dBlclick', [value, text, data]);
            });
        } 
    });
...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            if(p.title){
            	g.title = $("<div class='tree-toolbar'>" +
            					"<div class='title'>"+p.title+"</div>" +
    							"<div class='clean'>清除</div>" +
            			"</div>");
            	
            	g.tree.before(g.title)
            }
            if(p.serach){
            	$(".l-box-select-inner").addClass('l-box-select-inner-search');
            	g.serach = $(
            			"<div class='tree-toolbar-lev2'>" +
            				"<div class='form-inline'>" +
            					"<input type='text' id='orgid_text' class='form-control'>" +
            					"<button class='l-button-box-search' id='l-search-btn' style='\"margin-left\":\"20px;\"'>搜索</button>" +
        					"</div>" +
    					"</div>" +
            			"<div class='tree-toolbar-tab'>" +
            				"<ul class='nav nav-tabs'>" +
            					"<li ><a conta='tree'>机构树</a></li>" +
            					"<li><a conta='search'>搜索结果</a></li>" +
            				"</ul>" +
            				"<div class='tab-content'>" +
            					"<div  class='tabpanel tab-pane tree-box' id='treelist'>" +
            					"</div>" +
            					"<div  class='tabpanel tab-pane tree-box' id='searchlist'>" +
            						"<div class='l-tree-search'></div>" +
        						"</div>" +
            				"</div>" +
        				"</div>");
            	g.tree.before(g.serach)
            	$("#treelist").append(g.tree);
